...
PMS is a complete real-time project and task management system. It combines a responsive React workspace, a protected Express REST API, local JSON persistence, JWT authentication, Socket.IO synchronization, offline task support, concurrent-edit protection, tests, and Docker.
...
- Lightweight local JSON persistence with no database server
...
Open `http://localhost:8080`. Application data is kept in the `pms_data` volume.

## Local development

Requirement: Node.js 20.19+. No database installation is required.
...
The server tests use an isolated local JSON data file and require no external services.

## Production values

- `DATA_FILE` (optional; defaults to `server/data/pms.json`)
