...
PMS is a complete real-time project and task management system. It combines a responsive React workspace, a protected Express REST API, MongoDB persistence, JWT authentication, Socket.IO synchronization, offline task support, concurrent-edit protection, tests, and Docker.
...
- Dedicated `/admin/login` route and separate administrator navigation
...
- GitHub Actions CI, tests, coverage, and production build
- Multi-stage Docker image, Docker Compose, and health check
...
The client uses one required Vite `index.html` entry document. React Router renders all member and administrator screens with clean paths such as `/home`, `/tasks`, `/board`, `/admin/dashboard`, and `/admin/members`. Navigation stays inside the React application without full-page reloads.

## Quick start with Docker

```bash
cp .env.example .env
# Edit .env and replace <db_username> and <db_password> in MONGO_URI.
docker compose up --build
```

Open `http://localhost:8080`. Application data is stored in the MongoDB database configured by `MONGO_URI`.

## Local development

Requirements: Node.js 20.19+ and a reachable MongoDB database, such as MongoDB Atlas.
...
The server tests require a reachable MongoDB database configured by `MONGO_URI`.

## Production values

- `MONGO_URI` (required; your MongoDB or MongoDB Atlas connection string)
- `MONGO_DATABASE` (optional; defaults to `pms`)
