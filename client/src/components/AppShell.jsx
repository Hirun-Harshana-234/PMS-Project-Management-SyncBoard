  ...
  return <div className="app-shell">
    <aside className={`sidebar ${menuOpen ? "open" : ""}`}>
      ...
      <nav className="main-nav" aria-label="Member navigation">
        {nav.map((item) => <NavLink key={item.to} to={item.to} onClick={() => setMenuOpen(false)}><Icon name={item.icon} /><span>{item.label}</span>{item.to === "/notifications" && unread > 0 && <b className="nav-badge">{Math.min(unread, 99)}</b>}{item.to === "/contact" && unreadMessages > 0 && <b className="nav-badge">{Math.min(unreadMessages, 99)}</b>}</NavLink>)}
      </nav>
