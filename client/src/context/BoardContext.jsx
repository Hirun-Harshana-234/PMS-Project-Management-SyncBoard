...
const initialState = {
  boards: [], board: null, tasks: [], activities: [], messages: [], onlineUserIds: [], loading: true,
...
export function boardReducer(state, action) {
  switch (action.type) {
    ...
    case "boardLoaded": return { ...state, board: action.board, tasks: action.tasks, activities: action.activities || [], messages: [], loading: false, isOffline: false };
    ...
    case "messagesLoaded": return { ...state, messages: action.messages.reduce((items, message) => mergeById(items, message), state.messages) };
    case "messageAdded": return action.message?.boardId && state.board?.id && action.message.boardId !== state.board.id ? state : { ...state, messages: mergeById(state.messages, action.message) };
    case "messageUpdated": return action.message?.boardId && state.board?.id && action.message.boardId !== state.board.id ? state : { ...state, messages: mergeById(state.messages, action.message) };
...
export function BoardProvider({ children }) {
  ...
  useEffect(() => {
    if (!user || !state.board?.members?.some((member) => member.user.id === user.id)) return;
    const members = state.board.members.map((member) => member.user.id === user.id ? { ...member, user } : member);
    dispatch({ type: "boardUpserted", board: { ...state.board, members } });
  }, [user]);
  ...
  useEffect(() => {
    ...
    socket.on("message:created", ({ message, boardId }) => dispatch({ type: "messageAdded", message: { ...message, boardId } }));
    socket.on("message:read", ({ message, boardId }) => dispatch({ type: "messageUpdated", message: { ...message, boardId } }));
  ...
  async function loadMessages(withUser = "") {
    const query = withUser ? `?with=${encodeURIComponent(withUser)}` : "";
    const data = await api.get(`/boards/${stateRef.current.board.id}/messages${query}`);
    dispatch({ type: "messagesLoaded", messages: data.messages || [] });
    return data.messages || [];
  }

  async function sendMessage(recipientId, body, kind = "message") {
    const data = await api.post(`/boards/${stateRef.current.board.id}/messages`, { recipientId, body, kind });
    dispatch({ type: "messageAdded", message: data.message });
    return data.message;
  }

  async function markMessageRead(messageId) {
    const data = await api.patch(`/boards/${stateRef.current.board.id}/messages/${messageId}/read`, {});
    dispatch({ type: "messageUpdated", message: data.message });
    return data.message;
  }
  ...
  const value = useMemo(() => ({
    ...
    addMember, removeMember, loadMessages, sendMessage, markMessageRead, clearConflict: () => dispatch({ type: "clearConflict" })
