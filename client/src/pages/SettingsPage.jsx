...
export default function SettingsPage({ adminMode = false }) {
  ...
  return <section className="content-section narrow settings-page">
    ...
    <div className="settings-grid">
      ...
      <section className="panel settings-section account-settings"><header><span className="settings-icon"><Icon name={adminMode ? "admin" : "profile"} /></span><div><h3>{adminMode ? "Administrator account" : "Account"}</h3><p>Current authenticated identity and access level.</p></div></header><dl><div><dt>Name</dt><dd>{user.displayName}</dd></div><div><dt>Username</dt><dd>{user.username}</dd></div><div><dt>Role</dt><dd>{user.role}</dd></div><div><dt>Department</dt><dd>{user.department || "Project Team"}</dd></div></dl>{!adminMode && <Link className="button secondary" to="/profile.html">Edit profile details</Link>}</section>
    </div>
