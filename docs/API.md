...
| PATCH | `/auth/profile` | Update display name, email, avatar color, or compressed profile image |

Registration body: `{ "displayName", "username", "email", "password" }`.

Profile image updates use `avatarData`, a resized PNG, JPG, or WebP data URL from the client.
...
| Method | Endpoint | Purpose |
| --- | --- | --- |
| GET | `/boards/:boardId/messages` | List messages involving the signed-in member; use `?with=userId` for one conversation |
| POST | `/boards/:boardId/messages` | Send a `message` or `help` request to another board member |
| PATCH | `/boards/:boardId/messages/:messageId/read` | Mark an incoming message as read |

Message body: `{ "recipientId", "body", "kind": "message" | "help" }`.
...
| POST | `/admin/users` | Create a member with a username/password and optional board access |
