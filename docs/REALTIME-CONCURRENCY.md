...
After JWT authentication, each Socket.IO connection joins a private user room and every board room that user may access. Controllers emit board/task events only after the local data store confirms the write. Clients merge events by record ID, so a writer receiving its own event does not create duplicates.
