...
async function listMine(req, res) {
  const requests = await AccessRequest.find({ requester: req.user._id }).populate("reviewedBy", "displayName username email role jobTitle department progress active avatarColor avatarData lastSeenAt createdAt").sort({ createdAt: -1 });
  res.json({ requests: requests.map(mapRequest) });
}

async function listAll(_req, res) {
  const requests = await AccessRequest.find().populate("requester", "displayName username email role jobTitle department progress active avatarColor avatarData lastSeenAt createdAt").populate("reviewedBy", "displayName username email role jobTitle department progress active avatarColor avatarData lastSeenAt createdAt").sort({ createdAt: -1 });
  res.json({ requests: requests.map(mapRequest) });
}
...
async function reviewRequest(req, res) {
  ...
  await request.save();
  const populated = await AccessRequest.findById(request._id).populate("requester", "displayName username email role jobTitle department progress active avatarColor avatarData lastSeenAt createdAt").populate("reviewedBy", "displayName username email role jobTitle department progress active avatarColor avatarData lastSeenAt createdAt");
