...
const teamProfiles = [
  ...
  { displayName: "Kavindu Perera", username: "kavindu.backend", email: "kavindu@pms.local", jobTitle: "Backend Developer", department: "Engineering", progress: 68, avatarColor: "#22bfd1" },
  { displayName: "Sahan Fernando", username: "sahan.devops", email: "sahan@pms.local", jobTitle: "DevOps Engineer", department: "Infrastructure", progress: 57, avatarColor: "#f1a53a" },
  { displayName: "Dinithi Jayasinghe", username: "dinithi.frontend", email: "dinithi@pms.local", jobTitle: "Frontend Developer", department: "Engineering", progress: 74, avatarColor: "#39bd84" },
  { displayName: "Tharindu Wijesinghe", username: "tharindu.qa", email: "tharindu@pms.local", jobTitle: "QA Engineer", department: "Quality Assurance", progress: 63, avatarColor: "#e76574" }
];
...
async function seedDemo() {
  if (!process.env.MONGO_URI) throw new Error("MONGO_URI is required. Add your MongoDB connection string to .env.");
  await initStore();
  await ensureDemoWorkspace();
}

if (require.main === module) {
  seedDemo().then(async () => { await closeStore(); console.log("PMS demo accounts and project data are ready."); }).catch(async (error) => {
    console.error(error);
    await closeStore();
    process.exit(1);
  });
