...
function validateProfileImage(value) {
  ...
  if (typeof value !== "string" || value.length > 500000) {
    throw new AppError(422, "Profile image must be a compressed image under 500 KB.");
  }
  if (!/^data:image\/(png|jpe?g|webp);base64,[A-Za-z0-9+/=]+$/.test(value)) {
    throw new AppError(422, "Profile image must be a PNG, JPG, or WebP image.");
  }
  return value;
}
...
module.exports = { emailPattern, requiredString, validateRegistration, validateProfileImage, pick };
